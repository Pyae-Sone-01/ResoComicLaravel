...
@section('breadcrumb-info', __('backend.permission.permission_list'))
@section('content')
    <div class="container">
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header border-0"> 
                        <h3 class="card-title">
                            {{ __('backend.permission.permissions') }}
                        </h3>
                        <div class="card-toolbar">
                            <a href="{{ url('/admin/permission/create') }}" class="btn btn-primary " title="Add New User">
                                <i class="bi bi-plus-lg"></i> {{ __('backend.common.add_new') }}
                            </a>
                        </div>
                    </div>
                    <form method="get" action="{{ route('admin.permission.index') }}" id="permission-form" class="filter-clear-form">
                        <div class="card-header border-0">
                            ...
                            <table class="table align-middle table-row-bordered fs-6 gy-5 dataTable no-footer">
                                <thead>
                                    <tr class="text-start text-gray-600 fw-boldest fs-7 text-uppercase gs-0">
                                        ...
                                        <th class="min-w-150px">{{ __('backend.permission.name') }}</th>
                                        <th class="min-w-150px">{{ __('backend.permission.label') }}</th>
                                ...
                                <tbody class="fw-bold text-gray-600">
                                    @foreach($data as $item)
                                        <tr>
                                            ...
                                            <td>
                                                <a href="{{ url('/admin/permission/' . $item->id . '/edit') }}" title="Edit User">
                                                ...
                                                <form method="POST" action="{{ url('/admin/permission', $item->id) }}" class="deleteForm" style="display: inline;">                
