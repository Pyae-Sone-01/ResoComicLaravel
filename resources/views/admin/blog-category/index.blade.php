            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header border-0">
                        <h3 class="card-title">
                            {{ __('backend.sidebar.blog_category') }}
                        </h3>
                        <div class="card-toolbar">
                            <a href="{{ route('admin.blog-category.create') }}" class="btn btn-primary " title="Add New User">
                                <i class="bi bi-plus-lg"></i> {{ __('backend.common.add_new') }}
                            </a>
                        </div>
                    </div>
                    <form method="get" action="{{ route('admin.blog-category.index') }}" id="users-form" class="filter-clear-form">
                        <div class="card-header border-0">
                                ...
                                <div class="filter-section d-flex align-items-center position-relative my-1 me-3">
                                    <div class="input-group input-group">
                                        <input type="text" id="search" class="search-box form-control form-control-solid" aria-label="Sizing example input" name="search"
                                            ...
                                            @isset( request()->search ) value="{{ request()->get('search') }}" @endisset style="background-color: #F3F6F9;">
                                            @if(request()->get('search'))<i class="bi bi-x" onclick="clearSearch()" style="position: absolute; top: 11px; right: 70px; font-size: 22px;"></i>@endif
                            ...
                            <table class="table align-middle table-row-bordered fs-6 gy-5 dataTable no-footer">
                                <thead>
                                    <tr class="text-start text-gray-600 fw-boldest fs-7 text-uppercase gs-0">
                                        ...
                                        <th class="min-w-150px">{{ __('backend.blog_category.name') }}</th>
                                ...
                                <tbody class="fw-bold text-gray-600">
                                    @foreach($data as $item)
                                        <tr>
                                            ...
                                            <td>{{ $item->{'title_' . session("locale")} }}</td>
                                            <td>
                                                <div class="form-check form-switch">
                                                    <input class="form-check-input" id="switcher_checkbox_{{ $item->id }}"
                                                        type="checkbox" onclick="statusChange({{ $item->id }}, '/admin/blog-category/update-status')"
                                                        name="status{{ $item->id }}"
                                                        {{ $item->status ? 'checked' : '' }}>
                                                </div>
                                            </td>
                                            ...
                                            <td class="text-center">
                                                <a href="{{ route('admin.blog-category.edit',$item) }}" title="Edit Blog Category">
                                                ...
                                                <form method="POST" action="{{ route('admin.blog-category.destroy', $item) }}" class="deleteForm" style="display: inline;">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.updatesort').click(function(){
                ...
                axios({
                    method: 'post',
                    url   : '/admin/blog-category/update-sort',
