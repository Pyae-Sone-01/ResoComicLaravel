...
@section('breadcrumb-info', __('backend.user.users'))
@section('content')
    <div class="container">
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header border-0"> 
                        ...
                        <div class="card-toolbar">
                            <a href="{{ route('admin.user.create') }}" class="btn btn-primary " title="Add New User">
                                <i class="bi bi-plus-lg"></i> {{ __('backend.common.add_new') }}
                            </a>
                        </div>
                    </div>
                    <form method="get" action="{{ route('admin.user.index') }}" id="users-form" class="filter-clear-form">
                        <div class="card-header border-0">
                            ...
                            <table class="table align-middle table-row-bordered fs-6 gy-5 dataTable no-footer">
                                <thead>
                                    <tr class="text-start text-gray-600 fw-boldest fs-7 text-uppercase gs-0">
                                        ...
                                        <th class="min-w-150px">{{ __('backend.user.name') }}</th>
                                        <th class="min-w-150px">{{ __('backend.user.email') }}</th>
                                        <th class="min-w-150px">{{ __('backend.user.role') }}</th>
                                ...
                                <tbody class="fw-bold text-gray-600">
                                    @foreach($data as $item)
                                        <tr>
                                            ...
                                            <td>
                                                <a href="{{ route('admin.user.edit',$item) }}" title="Edit User">
                                                ...
                                                <form method="POST" action="{{ route('admin.user.destroy', $item) }}" class="deleteForm" style="display: inline;">                
